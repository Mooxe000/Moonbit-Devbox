...
type PageMode =
    ...
    member this.UrlPath =
        match this with
        | WASM -> "wasm" // This should be synced with the StaticWebAssetBasePath which defined in CoolMixMode.Wasm.fsproj file
        | SERVER -> "server"
...
module Pages =
    ...
    let create (mode: PageMode) ctx =
        let rootView = rootComp<CoolMixMode.Wasm.App.AppComp> ctx mode.RenderMode
